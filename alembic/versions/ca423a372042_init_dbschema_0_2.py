...
Revision ID: ca423a372042
Revises: 
Create Date: 2024-11-12 21:35:35.228646
...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_dbuser_name'), 'dbuser', ['name'], unique=True)
    ...
    op.create_index(op.f('ix_dbgroup_name'), 'dbgroup', ['name'], unique=True)
    ...
    op.create_index(op.f('ix_dbpymolcmd_name'), 'dbpymolcmd', ['name'], unique=True)
    ...
    op.create_index(op.f('ix_dbworkflow_name'), 'dbworkflow', ['name'], unique=True)
    op.create_table('useruserdefaultlink',
    sa.Column('followersid', sa.Uuid(), nullable=False),
    sa.Column('followingid', sa.Uuid(), nullable=False),
    sa.ForeignKeyConstraint(['followersid'], ['dbuser.id'], ),
    ...
    op.create_index(op.f('ix_dbflowstep_name'), 'dbflowstep', ['name'], unique=True)
    ...
    op.create_index(op.f('ix_dbpoll_name'), 'dbpoll', ['name'], unique=True)
    op.create_table('groupuserdefaultlink',
    sa.Column('groupsid', sa.Uuid(), nullable=False),
    sa.Column('usersid', sa.Uuid(), nullable=False),
    sa.ForeignKeyConstraint(['groupsid'], ['dbuser.id'], ),
    sa.ForeignKeyConstraint(['usersid'], ['dbgroup.id'], ),
    ...
    sa.ForeignKeyConstraint(['filekindid'], ['dbfilekind.id'], ),
    sa.ForeignKeyConstraint(['parentid'], ['dbpollfile.id'], ),
    ...
    sa.Column('comment', sqlmodel.sql.sqltypes.AutoString(), nullable=False),
    sa.Column('durationsec', sa.Integer(), nullable=False),
    sa.ForeignKeyConstraint(['flowstepid'], ['dbflowstep.id'], ),
    sa.ForeignKeyConstraint(['reviewid'], ['dbreview.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_dbpoll_name'), table_name='dbpoll')
    op.drop_table('dbpoll')
    op.drop_index(op.f('ix_dbflowstep_name'), table_name='dbflowstep')
    op.drop_table('dbflowstep')
    op.drop_table('useruserdefaultlink')
    op.drop_index(op.f('ix_dbworkflow_name'), table_name='dbworkflow')
    op.drop_table('dbworkflow')
    op.drop_index(op.f('ix_dbpymolcmd_name'), table_name='dbpymolcmd')
    op.drop_table('dbpymolcmd')
    op.drop_index(op.f('ix_dbgroup_name'), table_name='dbgroup')
    op.drop_table('dbgroup')
    op.drop_index(op.f('ix_dbuser_name'), table_name='dbuser')
